...
create.boxplot.facet <- function(df, outcome.var) {
 # pivot data to long form
df.long <- df %>%
  pivot_longer(-c(name, mpg, outcome.var), names_to = "Predictor", values_to = "Value")
...
class.boxplot <- create.boxplot.facet(test.df, test.vars)
class.boxplot
...
set.seed(1)
Auto.knn.preds <- knn(Auto.train.knn.x, Auto.test.knn.x, cl=Auto.train.knn.y, k=6)
# conf matrix
...
```
KNN sucks for this. Clearly. Best option was LDA.
