...
# pivot data to long form
df.long <- df %>%
  pivot_longer(-c(mpg01, name, mpg), names_to = "Predictor", values_to = "Value")
df.long <- df.long[, c("Predictor", "mpg01", "Value")] # reorder columns

# create facet of plots
boxplot <- ggplot(df.long, aes(x=Predictor, y=Value, fill=mpg01)) +
  geom_boxplot() +
  facet_wrap(~Predictor, scales = "free")
boxplot
