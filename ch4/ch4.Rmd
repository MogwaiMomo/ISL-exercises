...
run.classification.model <- function(train.test.list){
  ...
  # if(missing(omit.vars)) {
  #   remove.vars <- outcome.var
  # } else {
  #   remove.vars <- c(outcome.var, omit.vars)
  # }
  
  # set data to model and test with
  train.df <- train.test.list[["train"]]
  test.df <- train.test.list[["test"]]
  test.outcomes <-  train.test.list[["test"]]$crime_class
  ...
  if (model.choice == "1") {
    ...
    model.fit <- glm(crime_class ~ . - crim - chas, data = train.df, family=binomial)
    summary(model.fit)
    # model.fit.probs <- predict(model.fit, newdata = test.outcomes, type ="response")
    # model.fit.preds <- rep("low", length(model.fit.probs))
    # model.fit.preds[model.fit.probs > 0.5] = "high"
    # model.fit.preds

  } else if (model.choice == "2") {
    # run LDA
      model.fit <- lda(crime_class ~ . - crim - chas, data = train.df)
      model.fit.preds <- predict(model.fit, newdata = test.outcomes)$class
      model.fit.preds
      
  } else if (model.choice == "3") {
    # run QDA
      model.fit <- qda(crime_class ~ . - crim - chas, data = train.df)
      model.fit.preds <- predict(model.fit, newdata = test.outcomes)$class
      model.fit.preds
    ...
    # set predictors - !!!NEED TO SCALE !!!
    X.train <- train.df[, -c(chas, crim, crime_class)]
    X.test <- test.df[, -c(chas, crim, crime_class)]
    
    # set outcomes
    Y.train <- train.df$crime_class
    Y.test <- train.df$crime_class
  
    set.seed(1)
    model.fit <- knn(X.train, X.test, Y.train, k = 1)
    model.fit
  }
...
# df <- Boston
# outcome.var <- "crime_class"
# omit.vars <- c("chas", "crim")
